...
runuser -l $LIBPANDA_USER -c /etc/libpanda.d/apps/teamduo/installRosPackages.sh
...
rosrun robot_upstart install teamduo/launch/${LAUNCH_FILE} --user root

echo "Enabling can_to_ros startup script"
sudo systemctl daemon-reload
sudo systemctl enable teamduo
popd
echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
